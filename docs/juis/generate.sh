...
echo
echo -e '# Links from AVM Juis for FOS from HWR 150 onward, plus Dect and BPjM'
echo -e '  - AVM uses insecure http:// links, so the file signatures must be verified before use.'
echo -e '  - If different links are shown for a device, the data returned by Juis is inconsistent.'
echo -e '  - Lab builds do all sorts of things with personal data that you must first agree to electronically.'
echo -e '  - Inhouse builds are special enough that they should not be used outside AVM.'
echo -e '  - In addition, all Inhouse+Lab links to AVM are mangled here to preserve AVM privacy and the like.'
echo -e '  - This list is neither complete, accurate, nor current.'
echo -e '\n### FOS-Release'  ; cat fos-xxx  | while read -s x; do echo "  - HWR ${x%=*}: [${x##*/}](${x#*=})"; done
echo -e '\n### FOS-Lab'      ; cat fos-lab  | while read -s x; do echo "  - HWR ${x%=*}: [${x##*/}](${x#*=})"; done
echo -e '\n### FOS-Inhouse'  ; cat fos-inh  | while read -s x; do echo "  - HWR ${x%=*}: [${x##*/}](${x#*=})"; done
echo -e '\n### Dect-Release' ; cat dect-rel | while read -s x; do echo "  - MHW ${x%=*}: [${x##*/}](${x#*=})"; done
echo -e '\n### Dect-Lab'     ; cat dect-lab | while read -s x; do echo "  - MHW ${x%=*}: [${x##*/}](${x#*=})"; done
echo -e '\n### Dect-Inhouse' ; cat dect-inh | while read -s x; do echo "  - MHW ${x%=*}: [${x##*/}](${x#*=})"; done
