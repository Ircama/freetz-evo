...
| **[Disk Management Web UI](docs/make/disk-mgmt.md)** (`disk-mgmt-cgi`) | Browser-based disk, partition, and filesystem management console comparable to GParted + Clonezilla: drag-and-drop partitioning, ext2/3/4 + FAT + exFAT + NTFS formatting, filesystem-aware cloning via partclone, ddrescue imaging, network send/receive, full disk migration, SMART/hdparm diagnostics, GPT analysis, operation queue with command preview, dry-run mode, real-time streaming output, multi-language UI (en/it/de/fr/es). See [docs/make/disk-mgmt.md](docs/make/disk-mgmt.md). | EVO only |
...
| **Disk tools** | Complete storage management ecosystem: `parted` 3.6, `gptfdisk` 1.0.10, `e2fsprogs`/`e2fsprogs-ng` 1.47.3, `dosfstools` 4.2, `exfatprogs` 1.3.2, `ntfs-3g` 2025.2.28, `fatresize`, `partclone` 0.3.31, `ddrescue` 1.30, `smartmontools` 7.5, `hdparm` 9.75, `util-linux` 2.41, `testdisk` 7.2, `fsarchiver` 0.8.9, `clonezilla` 5.15.23, `udpcast` 20250223. All packages are unified under the Disk Tools menuconfig category and work as backends for the [Disk Management Web UI](docs/make/disk-mgmt.md). |
