...
| **[`PostgreSQL libpq`](docs/make/postgresql.md)** 16.3 | PostgreSQL client-library package exporting `libpq` for target-side consumers such as SQL backends; no PostgreSQL server components are built for target. | EVO only |
| **[`PowerDNS`](docs/make/powerdns.md)** 5.0.5 | Authoritative DNS server port with optional `pdnsutil`, `pdns_control`, `zone2sql`, `zone2json`, `ixfrdist`, diagnostic tools, externalization support, selectable OpenSSL/GnuTLS DNS-over-TLS, optional libsodium and remote ZeroMQ support, and selectable `bind`/`pipe`/`godbc`/`gmysql`/`gpgsql`/`gsqlite3`/`geoip`/`lmdb`/`lua2`/`remote`/`tinydns` backends. Requires modern targets because upstream PowerDNS 5 needs GCC 8+ / C++17. | EVO only |
