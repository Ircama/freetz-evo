...
// For 64-bit aarch64, stat64 has the same layout as stat.
...
s! {
    ...
    // aarch64 uclibc bits/stat.h (64-bit): st_atim is a timespec in C, but std
    // accesses st_atime/st_atime_nsec (C macros expanding to tv_sec/tv_nsec), so
    // we expose them as separate i64 fields with identical ABI.
    pub struct stat {
...
// __SIZEOF_PTHREAD_COND_T is provided by linux_l4re_shared for linux+uclibc
// (gated `not(target_os = "l4re")`), so defining it here would collide.
...
pub const FIOQSIZE: crate::c_ulong = 0x545E;
...
// aarch64 syscall numbers (from <asm-generic/unistd.h>, same for uclibc and gnu)
...
// aarch64 hwcap / eventfd constants (from libc gnu/b64/aarch64)
...
// EFD_SEMAPHORE is provided by linux/mod.rs for linux+uclibc

// Additional aarch64 constants (values from libc gnu/b64/aarch64, no collision)
pub const SFD_CLOEXEC: c_int = 0x080000;
pub const SFD_NONBLOCK: c_int = 0x0800;
// SOCK_CLOEXEC is provided by linux_like/mod.rs for linux+uclibc
