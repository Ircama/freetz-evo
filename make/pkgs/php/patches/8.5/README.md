...
### ~~500-ttyname_r.patch~~ (REMOVED - obsolete in 8.5)
**Status**: Removed. PHP 8.5 no longer needs this patch.
**Why**: Upstream `ext/posix/config.m4` now natively defines `HAVE_TTYNAME_R` via
`AS_VAR_IF([php_cv_func_ttyname_r], [yes], [AC_DEFINE([HAVE_TTYNAME_R], [1], ...)])`
when the check passes, so the generated `configure` already emits
`#define HAVE_TTYNAME_R 1`. The old patch's context no longer exists in the
regenerated autoconf output and would fail to apply.
