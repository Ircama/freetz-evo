...
$(PKG)_SOURCE_DOWNLOAD_NAME:=v$(RMPC_VERSION).tar.gz
$(PKG)_SOURCE:=$(pkg)-$(RMPC_VERSION).tar.gz
...
$(PKG)_DIR:=$(SOURCE_DIR)/rmpc-$(RMPC_VERSION)
...
$(PKG)_TARGET_BINARY:=$(RMPC_DEST_DIR)/usr/bin/rmpc
...
$(RMPC_BINARY): $(RMPC_DIR)/.configured
	cd $(RMPC_DIR); \
...
$(pkg)-precompiled: $(RMPC_TARGET_BINARY)

$(RMPC_TARGET_BINARY): $(RMPC_BINARY)
	$(INSTALL_BINARY_STRIP)

$(pkg)-clean:
	cd $(RMPC_DIR) && cargo clean 2>/dev/null || true
	$(RM) $(RMPC_BINARY) $(RMPC_DIR)/.configured $(RMPC_DIR)/.cargo/config.toml
