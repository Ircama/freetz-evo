...
$(PKG)_SOURCE:=$(pkg)-$(GITUI_VERSION).tar.gz
...
$(PKG)_TARGET_BINARY:=$(GITUI_DEST_DIR)/usr/bin/gitui
...
$(GITUI_BINARY): $(GITUI_DIR)/.configured
	cd $(GITUI_DIR); \
...
$(eval $(call INSTALL_BINARY_STRIP_RULE,$(GITUI_BINARY),/usr/bin))

$(pkg):

$(pkg)-precompiled: $(GITUI_TARGET_BINARY)

$(pkg)-clean:
	-$(SUBMAKE) -C $(GITUI_DIR) clean
	$(RM) $(GITUI_BINARY) $(GITUI_DIR)/.configured $(GITUI_DIR)/.cargo/config.toml

$(pkg)-uninstall:
	$(RM) $(GITUI_TARGET_BINARY)
