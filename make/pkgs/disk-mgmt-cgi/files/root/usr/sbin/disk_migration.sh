...
DESCRIPTION
    ...
    Logical mode (default):
      ...
      on the target disk at a proportionally scaled offset, and clones the
      data using partclone (smart or dd).  The target disk may be smaller
      than the source as long as the actually-used sectors fit.
...
if [ "$TGT_TOTAL" -lt "$SRC_TOTAL" ]; then
    # Use integer arithmetic: scale_num / scale_den = TGT_TOTAL / SRC_TOTAL
    SCALE_NUM="$TGT_TOTAL"
    SCALE_DEN="$SRC_TOTAL"
    echo "     Target smaller than source: proportional scaling applied."
...
while IFS=' ' read -r _pnum _pstart _pend _psize _pfs _pname; do
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        ...
        # Resolve target partition node
        _tgt_part=''
        _try=0
        while [ "$_try" -lt 5 ] && [ -z "$_tgt_part" ]; do
        ...
        if [ -z "$_tgt_part" ]; then
            echo "     ⚠️  Could not resolve target partition node; trying sequential guess."
            _tgt_part=$(partition_path "$TARGET_DEVICE" "$_pnum")
        fi
