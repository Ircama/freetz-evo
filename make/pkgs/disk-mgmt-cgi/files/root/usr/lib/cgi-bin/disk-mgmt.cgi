...
(function () {
	...
	function findContainingFreeSegment(dev, rangeStart, rangeEnd) {
		...
		for (var i = 0; i < dev.partitions.length; i++) {
			...
			if (!seg || seg.kind !== 'free') continue;
			var segStart = Number(seg.start || 0);
			var segEnd = Number(seg.end || 0);
			if (!isFinite(segStart) || !isFinite(segEnd) || segEnd < segStart) continue;
			if (start >= segStart && end <= segEnd) return seg;
		}
		return null;
	}
	...
	var FIELD_HELP = {
		...
		'fritz-table-type':  { title: 'Partition table',   body: '<b>GPT</b> – GUID Partition Table. Supports disks larger than 2 TiB, up to 128 partitions (all equal — no primary/logical distinction), and is required for UEFI boot. Recommended for all modern disks.<br><br><b>msdos / MBR</b> – Master Boot Record. Legacy format with a 2 TiB disk limit and exactly 4 partition <em>slots</em> (numbered 1–4). Those slots can hold primary partitions or one extended partition.<br>• A <b>primary</b> partition occupies one slot directly.<br>• An <b>extended</b> partition occupies one slot but acts as a container for <b>logical</b> partitions (numbered ≥ 5). Logical partitions do <em>not</em> consume a primary slot, so you can have more than 4 partitions on MBR as long as one slot is used for the extended container.' },
							...
							_setTextSelectionSuppressed(false);
                                ...
                                if (_ctSrcInExt !== _ctTgtInExt) {
                                    ...
                                    showMovePartModal(String(_bdDev.path || ''), _bdPart, String(_bdDev.path || ''),
										_newStart, _newEnd,
										Number(_targetFreeSeg.start || _newStart),
										Number(_targetFreeSeg.end || _newEnd));
                                } else {
                                    /* Same-type: clear dragCtx once modal chain resolves (cancel or queue) */
									var _p = queueSfdiskMovePartition(_bdPart, String(_bdDev.path || ''), _newStart, { requireFreeTarget: true });
					...
					block.ondrop = function (ev) {
						...
						if (data === 'new-partition') {
                            ...
                            if (_sameType) {
                                ...
                                renderMap();
								var _chipMoveP = queueSfdiskMovePartition(moveSource, srcDevicePath, targetStart, { requireFreeTarget: true });
                                ...
                                showMovePartModal(
                                    ...
                                    targetStart,
                                    targetEnd,
									Number(_targetFreeSeg.start || targetStart),
									Number(_targetFreeSeg.end   || targetEnd)
                                );
                            }
						}
					};
...
function showMovePartModal(targetDevPath, srcPart, srcDevPath, targetStart, targetEnd, freeStart, freeEnd) {
	...
	okBtn.onclick = function () {
		...
		var tgtPreviewDev = getPreviewDeviceByPath(tgtDev);
		if (!findContainingFreeSegment(tgtPreviewDev, tStart, tEnd)) {
			showToast(t('tMoveNoSpace'), 'error', 10000);
			return;
		}
...
showToast(t('tQueued') + ' ' + deleteLabel, 'info', 10000);
	...
	function queueSfdiskMovePartition(partArg, devPath, presetStart, opts) {
		var options = opts || {};
		var devP = String(devPath || state.selectedDevice || '');
		var part = partArg || state.selectedPart;
