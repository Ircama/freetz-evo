...
# Cap parallel V8 compilation jobs to avoid OOM on memory-constrained hosts.
# Each TQ-generated .cc compiled at -O3 can consume 1-3 GB; limit to
# floor(MemAvailableMB / 2048), clamped between 1 and 4.
NODEJS_MAX_JOBS := $(shell awk '/MemAvailable/{j=int($$2/1024/2048); if(j<1)j=1; if(j>4)j=4; print j}' /proc/meminfo 2>/dev/null || echo 4)
NODEJS_HOST_ARCHFLAGS := $(if $(NODEJS_USE_HOST_MULTILIB),-m32,)
NODEJS_HOST_CFLAGS := $(NODEJS_HOST_ARCHFLAGS)
NODEJS_HOST_CXXFLAGS := $(NODEJS_HOST_ARCHFLAGS) $(if $(NODEJS_USE_HOST_MULTILIB),-I$(NODEJS_HOST_MULTILIB_CXXINC),)
# ICU host tools can emit sized delete calls; disable sized deallocation to keep
# host helper linking robust across mixed multilib/libstdc++ environments.
NODEJS_HOST_CXXFLAGS += -fno-sized-deallocation
NODEJS_HOST_CPPFLAGS := $(if $(NODEJS_USE_HOST_MULTILIB),-I$(NODEJS_HOST_MULTILIB_CXXINC),)
NODEJS_HOST_LDFLAGS := $(NODEJS_HOST_ARCHFLAGS) $(if $(NODEJS_USE_HOST_MULTILIB),-L$(NODEJS_HOST_MULTILIB_LIBDIR),)
...
# When cross-compiling with Intl for a big-endian target, configure.py uses
# sys.byteorder (the HOST's endianness) to set icu_endianness — always 'l' on
# an x86 build machine even when the target is MIPS big-endian.  Patch it to
# hard-code 'b' so the build system generates icudt74b embedded data and dat file.
ifeq ($(FREETZ_PACKAGE_NODEJS_WITH_INTL)-$(FREETZ_TARGET_ARCH_BE),y-y)
$(PKG)_CONFIGURE_PRE_CMDS += grep -q "icu_endianness = 'b'" configure.py || \
	$(SED) -i -e "s/icu_endianness = sys.byteorder\[0\]/icu_endianness = 'b'/" configure.py;
endif
...
$(PKG)_CONFIGURE_ENV += CFLAGS_host="$(NODEJS_HOST_CFLAGS)"
$(PKG)_CONFIGURE_ENV += CXXFLAGS_host="$(NODEJS_HOST_CXXFLAGS)"
$(PKG)_CONFIGURE_ENV += CPPFLAGS_host="$(NODEJS_HOST_CPPFLAGS)"
$(PKG)_CONFIGURE_ENV += LDFLAGS_host="$(NODEJS_HOST_LDFLAGS)"
...
#include <stdarg.h>' $(NODEJS_DIR)/deps/v8/src/base/strings.h
  ...
  return 0;\
	...
	# On MIPS BE, the V8 snapshot blob is generated by the LE-host mksnapshot binary.
	# CopySlots() copies raw 4-byte heap slots from the snapshot via memcpy without any
	# endian conversion. On a BE target, each slot is misread as byte-swapped, causing wrong
	# Smi values in the heap, deserializer stream desync, and UNREACHABLE() crashes.
	# Fix: byte-swap each AtomicWord slot after memcpy when V8_TARGET_BIG_ENDIAN is defined.
	grep -q 'V8_TARGET_BIG_ENDIAN' $(NODEJS_DIR)/deps/v8/src/snapshot/snapshot-source-sink.h || \
		$(SED) -i \
			-e '/memcpy(&val, data_ + position_, sizeof(base::AtomicWord));/a\
#if defined(V8_TARGET_BIG_ENDIAN)\
      val = static_cast<base::AtomicWord>(__builtin_bswap32(static_cast<uint32_t>(val)));\
#endif' \
			$(NODEJS_DIR)/deps/v8/src/snapshot/snapshot-source-sink.h
...
$($(PKG)_BINARY_TARGET): $($(PKG)_BINARY_BUILD)
	...
	if [ "$(FREETZ_PACKAGE_NODEJS_WITH_INTL)" = "y" ]; then \
		mkdir -p "$(NODEJS_DEST_DIR)/usr/share/icu"; \
		icu_data_file="$$(find $(NODEJS_DIR)/out/Release/obj/gen/icutmp $(NODEJS_DIR)/out/Release/obj/gen $(NODEJS_DIR)/deps/icu-tmp $(NODEJS_DIR)/out/Release -maxdepth 2 -type f -name 'icudt*.dat' 2>/dev/null | head -n 1)"; \
		if [ -z "$$icu_data_file" ]; then \
			echo "nodejs: ICU data file not found (expected icudt*.dat)" >&2; \
			echo "nodejs: searched in $(NODEJS_DIR)/out/Release/{obj/gen/icutmp,obj/gen,} and $(NODEJS_DIR)/deps/icu-tmp" >&2; \
			exit 1; \
		fi; \
		cp "$$icu_data_file" "$(NODEJS_DEST_DIR)/usr/share/icu/"; \
	fi
