...
$(PKG)_SOURCE:=$(pkg)-$(LNAV_RS_VERSION).tar.gz
...
$(PKG)_TARGET_BINARY:=$(LNAV_RS_DEST_DIR)/usr/bin/lnav-rs
...
$(LNAV_RS_BINARY): $(LNAV_RS_DIR)/.configured
	cd $(LNAV_RS_DIR); \
	...
	datafusion_common_dir="$$(find "$$CARGO_HOME/registry/src" -path '*/datafusion-common-53.1.0' -type d | head -n 1)"; \
	test -n "$$datafusion_common_dir" || exit 1; \
	grep -q 'not(target_env = "uclibc")' "$$datafusion_common_dir/src/rounding.rs" || \
		patch -N -d "$$datafusion_common_dir" -p1 < "$(abspath make/pkgs/lnav-rs/registry-patches/011-datafusion-common-no-fenv-uclibc.patch)"; \
...
$(eval $(call INSTALL_BINARY_STRIP_RULE,$(LNAV_RS_BINARY),/usr/bin))

$(pkg):

$(pkg)-precompiled: $(LNAV_RS_TARGET_BINARY)

$(pkg)-clean:
	-$(SUBMAKE) -C $(LNAV_RS_DIR) clean
	$(RM) $(LNAV_RS_BINARY) $(LNAV_RS_DIR)/.configured $(LNAV_RS_DIR)/.cargo/config.toml

$(pkg)-uninstall:
	$(RM) $(LNAV_RS_TARGET_BINARY)
