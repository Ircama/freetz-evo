...
static PyObject *mod_create_speex_noise(PyObject *, PyObject *args, PyObject *kwargs) {
    static const char *kwlist[] = {"auto_gain", "noise_suppression", NULL};
    double auto_gain;
    int noise_suppression;
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "di", (char **)kwlist, &auto_gain, &noise_suppression)) {
        return NULL;
    }
    ...
    if (noise_suppression != 0) {
        speex_preprocess_ctl(h->state, SPEEX_PREPROCESS_SET_NOISE_SUPPRESS, &noise_suppression);
    }

    int agc_enabled = (auto_gain > 0) ? 1 : 0;
    speex_preprocess_ctl(h->state, SPEEX_PREPROCESS_SET_AGC, &agc_enabled);
    if (auto_gain > 0) {
        speex_preprocess_ctl(h->state, SPEEX_PREPROCESS_SET_AGC_LEVEL, &auto_gain);
    }

    PyObject *capsule = PyCapsule_New((void *)h, CAPSULE_NAME, speex_noise_capsule_destructor);
    if (!capsule) {
        speex_preprocess_state_destroy(h->state);
        free(h);
        return NULL;
...
static PyObject *mod_process_10ms(PyObject *, PyObject *args, PyObject *kwargs) {
    static const char *kwlist[] = {"speex_noise", "audio", NULL};
    PyObject *cap = NULL;
    const char *data = nullptr;
    Py_ssize_t len = 0;

    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "Oy#", (char **)kwlist, &cap, &data, &len)) {
        return NULL;
    }
    ...
    int16_t *output_samples = reinterpret_cast<int16_t *>(PyBytes_AsString(output));

    Py_BEGIN_ALLOW_THREADS
    memcpy(output_samples, samples, BYTES_10_MS);
...
static PyMethodDef module_methods[] = {
    {"create_speex_noise", (PyCFunction)mod_create_speex_noise, METH_VARARGS | METH_KEYWORDS,
     "create_speex_noise(auto_gain: float, noise_suppression: int) -> capsule"},
    {"process_10ms", (PyCFunction)mod_process_10ms, METH_VARARGS | METH_KEYWORDS,
